...
class DepartmentController extends Controller
{
    public function index(Request $request)
    {
        if (Auth::user()->can('manage-departments')) {
            ...
            $employees = \App\Models\User::whereIn('created_by', getCompanyAndUsersId())
                ->where('type', 'employee')
                ->where('status', 'active')
                ->get(['id', 'name']);

            // Get all departments for parent select
            $allDepartments = Department::whereIn('created_by', getCompanyAndUsersId())
                ->where('status', 'active')
                ->get(['id', 'name']);
    ...
    public function store(Request $request)
    {
        if (Auth::user()->can('create-departments')) {
            $validated = $request->validate([
                ...
                'active_employees' => 'nullable|integer|min:0',
                'open_employees' => 'nullable|integer',
