...
export default function Employees() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (value: any, row: any) => {
        const managerName = row.employee?.manager?.name;
        if (!managerName) {
          return <span className="text-xs text-muted-foreground italic">{t('No Manager')}</span>;
        }
        return (
            ...
            {employees?.data?.map((employee: any) => (
              <Card key={employee.id} className="bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-lg shadow">
                ...
                <div className="p-6">
                  ...
                  {/* Department, Designation & Manager info */}
                  <div className="border border-gray-200 dark:border-gray-700 rounded-md p-3 mb-4 space-y-1">
                    ...
                    <div className="text-sm">
                      <span className="font-medium">{t("Reporting Manager")}:</span>{' '}
                      {employee.employee?.manager?.name ? (
                        <span className="text-indigo-600 dark:text-indigo-400 font-medium">{employee.employee.manager.name}</span>
                      ) : (
                        <span className="text-muted-foreground italic">{t('No Manager')}</span>
                      )}
                    </div>
                  </div>
